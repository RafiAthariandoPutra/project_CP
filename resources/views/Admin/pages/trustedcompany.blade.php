    ...
    <div data-dialog-backdrop="sign-in-modal" data-dialog-backdrop-close="true"
        ...
        <div data-dialog="sign-in-modal" data-dialog-mount="opacity-100 translate-y-0 scale-100"
            ...
            <div class="relative flex flex-col bg-white">
                ...
                <form class="w-full" action="/dashboard/trustedcompany" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="flex w-full flex-col gap-4 p-4">
                        ...
                        <div class="p-4 w-full">
                            <label class="block mt-4 text-sm">
                                <span class="text-slate-600 ">Company Logo</span>
                                <div class="flex items-center justify-center w-full">
                                    <label for="dropzone-file"
                                        ...
                                        <input id="dropzone-file" name="company_logo" onchange="previewImage()"
                                            id="image imgInp" value="{{ old('company_logo') }}" type="file"
                                            class="hidden image " />
                                    </label>
                                </div>
                    ...
                    <tbody>
                        @foreach ($trustedCompanies as $item)
                            <tr>
                                <td class="py-3 w-1/3 px-5 border-b border-blue-gray-50">
                                    <div class="flex items-center gap-4">
                                        <img src="/storage/{{ $item->company_logo }}" alt="{{ $item->company_name }}"
                                            class="w-full max-w-[100px] relative object-cover rounded-md" />
                                    </div>
                                </td>
                                <td class="py-3 w-1/3 px-5 border-b border-blue-gray-50">
                                ...
                                <td class="py-3 w-1/3 px-5 border-b border-blue-gray-50">
                                    <form action="/dashboard/trustedcompany/{{ $item->id }}" method="post"
                                        id="confirm">
                                        @method('delete')
                                        @csrf
